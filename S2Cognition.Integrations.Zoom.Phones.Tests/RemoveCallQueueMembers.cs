...
{
    public class RemoveCallQueueMembers : IntegrationTests
    {

        [Fact]
        public async Task EnsureRemoveCallQueuesMembersChecksQueueNameIsNotNull()
        {
            await _sut.Initialize(_configuration);

            var ex = await Should.ThrowAsync<ArgumentException>(async () => await _sut.RemoveCallQueueMembers(new RemoveCallQueueMemberRequest()));
        ...
        [Fact]
        public async Task EnsureRemoveCallQueuesMembersChecksUserEmailIsNotNull()
        {
            await _sut.Initialize(_configuration);

            var ex = await Should.ThrowAsync<ArgumentException>
                (async () => await _sut.RemoveCallQueueMembers(new RemoveCallQueueMemberRequest { QueueName = "fake Queue Name" }));
        ...
        [Fact]
        public async Task EnsureRemoveCallQueuesMembersExpectedResults()
        {
            ...
            var response = await _sut.RemoveCallQueueMembers(new RemoveCallQueueMemberRequest
            {
